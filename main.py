...
users:list = [{"name": "Łukasz", "surname": "XXX", "location": "Siedlce", "posts": 100,}]
...
def add_user():
    ...
    posty=entry_surname.get()
    miejscowosc=entry_surname.get()
    users.append(
        {
            "name": imie,
            "surname": nazwisko,
...
def user_details():
    ...
    label_location.configure(text=users[idx]["location"])
    label_posts.configure(text=users[idx]["posts"])
...
label_location=Label(ramka_szczegoly_obiektow, text="Miejscowość: ")
label_location.grid(row=1, column=6)
